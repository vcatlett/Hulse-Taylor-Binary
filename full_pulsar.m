...
gifname = 'Binary_System_Evolving.gif';
...
d_theta_p1 = 0.25;    % d theta for pulsar's orbit
d_r = 0;           %-dr for orbits
...
f_s1 = 50;         % rotation frequency (Hz) (true=16.9405)
...
for t = 1:1:360
    [pos1_p, pos2_p, shift1_p, shift2_p] = binary_orbit_evolving(t, p10, p20, ax_a, ax_b, m_d, com, 0, 0);
    x1p(t) = pos1_p(1);
    y1p(t) = pos1_p(2);
    z1p(t) = pos1_p(3);
...
% COM
com = [-(p10(1)+p20(1))/2, (p10(2)+p20(2))/2, 0];
...
for t = 0:1:360
    ...
    % Plot orbits
    %[x1p_r, y1p_r, z1p_r] = rotate(x1p, y1p, z1p, com, t, d_theta_p1, 'z');
    %[x2p_r, y2p_r, z2p_r] = rotate(x2p, y2p, z2p, com, 45, 1, 'y');
    [o1, o2] = full_orbit(p10, p20, ax_a, ax_b, m_d, com, t, d_theta_p1, d_r);
    %plot(x1p_r, y1p_r, 'cyan', 'LineWidth',1);
    %plot3(x2p, y2p, z2p, 'g', 'LineWidth',1);
    plot3(o1(:,1), o1(:,2), o1(:,3),'cyan', 'LineWidth',1);
    plot3(o2(:,1), o2(:,2), o2(:,3),'g', 'LineWidth',1);
    
    % Plot center of mass
    scatter(com(1), com(2), 25,'r','filled','r')
    
    % Move along the orbit
    [pos1, pos2, shift1, shift2] = binary_orbit_evolving(t, p10, p20, ax_a, ax_b, m_d, com, d_theta_p1, d_r);
